...
// --- FONTE EXPANDIDA (0:A, 1:G, 2:R, 3:O, 4:M, 5:I, 6:U, 7:S, 8:T, 9:K, 10:L, 11:E, 12:V) ---
const uint8_t micro_font[] = {
    ...
    0x46, 0x49, 0x49, 0x49, 0x31, 0x00, // S (7)
    0x01, 0x01, 0x7f, 0x01, 0x01, 0x00, // T (8)
    0x7f, 0x08, 0x14, 0x22, 0x41, 0x00, // K (9)
    0x7f, 0x40, 0x40, 0x40, 0x40, 0x00, // L (10)
    0x7f, 0x49, 0x49, 0x49, 0x41, 0x00, // E (11)
    0x1f, 0x20, 0x40, 0x20, 0x1f, 0x00  // V (12)
};
...
void som_forte_alerta() {
    // Frequência mais baixa e duty cycle maior para som "mais pesado"
    for(int i = 0; i < 60; i++) {
...
int main() {
    ...
    while (1) {
        ...
        if (desvio > 1600) alerta_ativo = true;
        ...
        // --- Linha 5: Telemetria Dinâmica ---
        oled_clear_line(5);
        oled_set_cursor(5, 35);

        if (alerta_ativo) {
            // VERMELHO: Crítico
            uint8_t a[] = {0, 10, 11, 2, 8, 0}; // ALERTA
            for(int i=0; i<6; i++) oled_write_char(a[i]);
            gpio_put(LED_RED_PIN, 1); gpio_put(LED_GREEN_PIN, 0);
            oled_command(0xA7); // Inverte
            som_forte_alerta();
            sleep_ms(50);
            oled_command(0xA6); // Normaliza
        } 
        else if (desvio >= 600) {
            // AMARELO: Aviso (Sem Buzzer)
            uint8_t av[] = {0, 12, 5, 7, 3}; // A V I S O
            ...
            // VERDE: Tudo OK
            oled_write_char(3); oled_write_char(9); // O K
